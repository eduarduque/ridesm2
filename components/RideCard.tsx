...
export default function RideCard({ ride, userId, hasRequested, onRequest, devMode, devRole }: Props) {
  ...
  function renderAction() {
    if (isOwn) return (
      <div className="w-full text-center py-2 bg-neutral-100 text-neutral-500 text-xs font-semibold rounded-lg">
        Your post
      </div>
    )
    if (!userId) return (
      <Link href="/login" className={`block w-full text-center py-2 rounded-lg text-xs font-bold text-white shadow-sm transition-colors ${isOffer ? 'bg-brand hover:bg-brand-dark' : 'bg-accent hover:bg-teal-700'}`}>
        Sign in to respond
      </Link>
    )
    if (ride.status === 'matched' || ride.status === 'expired' || ride.status === 'cancelled') return (
      <div className="w-full text-center py-2 bg-neutral-100 text-neutral-400 text-xs font-semibold rounded-lg">
        Closed
      </div>
    )
    ...
    return (
      <div className="flex gap-1.5">
        ...
        {phone && (
          <a
            href={`tel:${phone}`}
            className="px-3 py-2 rounded-lg border border-neutral-200 text-neutral-500 hover:bg-neutral-50 transition-colors flex items-center"
            title="Call"
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
  ...
  return (
    <div className={`bg-white border border-neutral-200/80 border-l-4 ${accentColor} rounded-xl overflow-hidden card-lift ${isUrgent ? 'ring-1 ring-red-400/20' : ''}`}>
      <div className="p-3 space-y-2">
        ...
        {/* Row 4: Note (if any) */}
        {ride.note && (
          <p className="text-[11px] text-neutral-500 italic line-clamp-1 px-2.5 py-1 rounded-lg bg-neutral-50 border border-neutral-100">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <div className={`w-4 h-4 rounded-full flex items-center justify-center text-white text-[8px] font-bold ${isOffer ? 'bg-brand' : 'bg-accent'}`}>
              {posterName[0].toUpperCase()}
            </div>
            <span className="text-[10px] text-neutral-400 font-medium">{posterName}</span>
          </div>
          <div className="flex items-center gap-1.5">
