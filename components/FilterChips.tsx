...
function Chip({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`shrink-0 px-4 py-2 rounded-full text-xs font-bold tracking-tight transition-all duration-150 active:scale-95 cursor-pointer border-2 ${
        active
          ? urgent
            ? 'bg-urgent text-white border-urgent'
            : 'bg-foreground text-background border-foreground'
          : urgent
          ? 'bg-urgent-light text-urgent border-transparent hover:border-urgent'
          : 'bg-surface text-foreground border-neutral-200 hover:border-foreground'
      }`}
    >
      {label}
    </button>
...
export default function FilterChips({
  ...
  return (
    <div className="sticky top-0 z-10 bg-background/90 backdrop-blur-md border-b border-neutral-200">
